...
const TierThree = ({ tierId }: any) => {
  ...
  const handleRefKey = async () => {
    ...
    try {
      ...
      const addwl = await contract.addWhitelistTierThree([walletAddress], {
        gasLimit: "2000000",
      });
  ...
  const handleRefKeyRemove = async (address: string) => {
    ...
    try {
      ...
      const addwl = await contract.removeWhitelistTierThree([address], {
        gasLimit: "2000000",
      });
      const response = await fetch(
  ...
  useEffect(() => {
    const getKey = async () => {
      ...
      setWl(data.result.tier_three);
    };
    getKey();
  }, [loading, success]);
  return (
    <ParentCard
      title="Add Tier 3 White List"
      footer={
        <>
