...
const TierFive = ({ tierId }: any) => {
  ...
  const handleRefKey = async () => {
    ...
    try {
      ...
      const addwl = await contract.addWhitelistTierFive([[walletAddress]], {
        gasLimit: "2000000",
      });
  ...
  const handleRefKeyRemove = async (address: string) => {
    ...
    try {
      ...
      const addwl = await contract.removeWhitelistTierFive([address], {
        gasLimit: "2000000",
      });
      const response = await fetch(
  ...
  useEffect(() => {
    const getKey = async () => {
      ...
      setWl(data.result.tier_five);
    };
    getKey();
  }, [success, loading]);
  return (
    <ParentCard
      title="Add Tier 5 White List"
      footer={
        <>
