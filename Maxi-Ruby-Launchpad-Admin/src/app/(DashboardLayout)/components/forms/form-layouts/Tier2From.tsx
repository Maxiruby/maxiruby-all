...
const TierTwo = ({ tierId }: any) => {
  ...
  const handleRefKey = async () => {
    ...
    try {
      ...
      const addwl = await contract.addWhitelistTierTwo([walletAddress], {
        gasLimit: "2000000",
      });
  ...
  const handleRefKeyRemove = async (address: string) => {
    ...
    try {
      ...
      const addwl = await contract.removeWhitelistTierTwo([address], {
        gasLimit: "2000000",
      });
      const response = await fetch(
  ...
  useEffect(() => {
    const getKey = async () => {
      ...
      setWl(data.result.tier_two);
    };
    getKey();
  }, [success, loading]);
  return (
    <ParentCard
      title="Add Tier 2 White List"
      footer={
        <>
